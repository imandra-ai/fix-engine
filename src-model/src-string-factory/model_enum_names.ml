...
let list_model_vals_OrderAttributeTypes = MOD_FIX_OrderAttributeTypes_AlgorithmicOrder :: (MOD_FIX_OrderAttributeTypes_LiquidityProvisionActivity :: []);;

let rec get_model_enum_vals_OrderAttributeTypes (f) (x)  =
    (match x with
        | [] -> []
        | h :: t -> (if (Map.get h f) then
            (h :: (get_model_enum_vals_OrderAttributeTypes f t))
        else
            (get_model_enum_vals_OrderAttributeTypes f t)
        )
    )
;;
