...
let check_field_f_ExecutionReport_Text ( msg_data : mod_executionreport_data ) =
    (match msg_data.f_ExecutionReport_Text with
        | None -> fields_are_good
        | Some _ -> fields_are_good
    )
;;

let check_fields_ExecutionReport ( msg_data , m_state : mod_executionreport_data * model_state ) =
    let f_ExecutionReport_Text_result = check_field_f_ExecutionReport_Text (msg_data) in
    if f_ExecutionReport_Text_result.is_invalid then
        f_ExecutionReport_Text_result
    else
