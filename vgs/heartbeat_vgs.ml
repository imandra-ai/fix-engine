...
verify hbeat_not_sent_if_duration_zero ( engine : fix_engine_state ) =
    let engine' = one_step ( engine ) in 
    let cond = 
        begin
...
let have_not_received_data' ( m, last_time_data_received, hbeat_interval, timereq_interval : fix_engine_int_msg option * fix_utctimestamp * fix_duration * fix_duration ) =
    match m with
    | Some ( TimeChange tc_data ) -> 
        begin 
            let valid_received_time = utctimestamp_duration_Add ( last_time_data_received, hbeat_interval ) in
...
verify testrequest_sent_correctly' ( engine : fix_engine_state ) =
    let engine' = one_step ( engine ) in 
    let cond =
        begin
            engine.fe_curr_mode = ActiveSession &&
...
verify non_garbled_updates_clock ( engine : fix_engine_state ) =
    ...
    ( incoming_is_not_garbled ( engine.incoming_fix_msg, engine.fe_curr_mode ) && 
        engine.fe_curr_mode <> CacheReplay && engine.fe_curr_mode <> Retransmit &&
...
let no_heartbeats_received_logoff ( m, engine : fix_engine_int_msg option * fix_engine_state ) =
    match m with
    | Some (TimeChange tc_data) -> 
...
let msg_is_test_request_id ( m, tr_id, seq_num : full_top_level_msg option * int * int ) =
    match m with
    | Some (ValidMsg vmsg) -> seq_num = vmsg.full_msg_header.h_msg_seq_num &&
        begin
            match vmsg.full_msg_data with 
