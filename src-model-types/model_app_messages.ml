...
(** These would be used to identify strings that are stored elsewhere. The reason why
    we need this reference is because we need a link to this string that
    would be printed outside the logic, but we need to cite it here. *)
let get_missing_field_str ( f : fix_field ) =
    match f with 
    | FIX_Field_OrigClOrdID _                   -> 1
    | FIX_Field_OrderID _                       -> 2
    | FIX_Field_ClOrdID _                       -> 3
    | FIX_Field_ExecType _                      -> 4
    | FIX_Field_ExecID _                        -> 5
    | FIX_Field_Account _                       -> 6
    | FIX_Field_Side _                          -> 7
    | FIX_Field_OrdType _                       -> 8
    | FIX_Field_OrdStatus _                     -> 9
    | FIX_Field_AvgPx _                         -> 10
    | FIX_Field_Price _                         -> 11
    | FIX_Field_LeavesQty _                     -> 12
    | FIX_Field_CumQty _                        -> 13
    | FIX_Field_HandlInst _                     -> 14
    | FIX_Field_TransactTime _                  -> 15
    | FIX_Field_Symbol _                        -> 16
    | FIX_Field_SymbolSfx _                     -> 17
    | FIX_Field_OrderQty _                      -> 18
    | FIX_Field_MinQty _                        -> 19        
    | FIX_Field_TimeInForce _                   -> 20
    | FIX_Field_Currency _                      -> 21
    | FIX_Field_LocateBroker _                  -> 22
    | FIX_Field_LocateReqd _                    -> 23
    | FIX_Field_ExecInst _                      -> 24
;;

let get_invalid_field_str ( f : fix_field ) = 
    match f with 
    | FIX_Field_OrigClOrdID _                   -> 1
    | FIX_Field_OrderID _                       -> 2
    | FIX_Field_ClOrdID _                       -> 3
...
type field_missing_data = {
    field_missing_data_field                        : fix_field_tag;
    field_missing_data_msg                          : fix_msg_tag;
}
;;
