...
let fixio_to_engine engine msg =
  ...
  Engine.send_fix engine msg >>= fun () ->
  Engine.send_getstate engine
...
let rec loop t =
  ...
  [ Lwt_mvar.take t.engine_box >>= 
    (function 
    | Engine.FIXMessage (Full_messages.ValidMsg msg) -> engine_to_fixio t.fixio msg 
    | Engine.Internal msg -> Model.send_fix t.model msg
    | Engine.State state -> save_state_seqns t.sessn state
    | _ -> Lwt.return_unit
    )
  ; ( Lwt_mvar.take t.fixio_box >>= fun msg ->
      fixio_to_engine t.engine msg ) 
  ; Lwt_mvar.take t.model_box >>= 
    (function 
    | Model.FIXMessage msg -> Engine.send_int t.engine msg 
    | _ -> Lwt.return_unit
    ) 
  ] >>= fun () -> loop t 
...
let with_catch_disconnect f =
  Lwt.catch f ( fun _  -> 
    ...
    Lwt_mvar.take established
  )

let f _ (inch, outch) =
  if not(Lwt_mvar.is_empty established) then Lwt_io.printl "Connection refused. Already running." else  
