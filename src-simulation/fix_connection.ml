...
let make_engine_state session_dir config =
  let s2f = String_utils.string_to_fix_string in
  let fe_comp_id            = s2f config.comp_id in 
  let fe_target_comp_id     = s2f config.target_id in    
  let fe_sender_location_id = 
    match config.host_id with None -> None 
    | Some host -> Some ( s2f host ) in  
