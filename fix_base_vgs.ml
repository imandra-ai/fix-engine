...
verify app_down_get_biz_reject ( state : fix_engine_state ) =
    ...
    no_incoming_msgs) ==> result_biz_reject
;;
...
let poss_dup_flag_not_set m = 
    match m with
    | None -> true
    | Some _ -> false
;;

verify smaller_seq_num_logout ( state : fix_engine_state ) =
    let msg_seq_reset x =
        match x.msg_data with 
        | FIX_sequence_reset _ -> true
        | _ -> false in
    
    let no_incoming_ints = 
    ...
    let incoming_msg_bad =
        ...
        | Some x ->
            ...
            ( not (msg_seq_reset (x)) || ( x.header.msg_seq_num < state.fe_curr_msg_seq_num ) ) in
    
    let state_good = not ( fengine.fe_application_up ) && fengine.fe_curr_mode = ActiveSession in
    ...
    ( no_incoming_ints && state_good && incoming_msg_bad ) ==> logout_sent
;;
