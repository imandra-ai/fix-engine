...
let check_begin_string channel =
  ...
  if str <> channel.begin_match_string then
    failwith "bad msg header"
  else
...
let read_chunk channel : (fix_channel, err) result =
  ...
  if nbytes = 0 then
    ...
    if channel.b_end < bsize - (2 * chunk_size) then
      Ok channel
    else (
...
let read_until channel char =
  ...
  let rec get_idx channel =
    try Result.ok @@ Bytes.index_from channel.buffer channel.b_start char
    with Not_found ->
...
let rec read_size size channel =
  if channel.b_end - channel.b_start < size then
    Ok channel
  else
    Result.bind (read_chunk channel) (read_size size)
