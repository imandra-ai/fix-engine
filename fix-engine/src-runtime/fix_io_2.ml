...
let create_channel (config:config) fd =
  ...
  { begin_match_string ; buffer ; fd 
  ; config ; b_start ; b_end 
  } 
...
let check_begin_string channel =
  ...
  if str <> channel.begin_match_string then failwith "bad msg header" else
  { channel with b_start = channel.b_start + lmatchstr }
...
let read_chunk channel : (fix_channel, err) result=
  ...
  if nbytes = 0 then Error `End_of_file else
  let channel = { channel with b_end = channel.b_end + nbytes } in
  let bsize = Bytes.length channel.buffer in
  if channel.b_end < bsize - 2 * chunk_size then Ok channel else 
...
let read_until channel char =
  ...
  let rec get_idx channel =
    try
      Result.ok @@ Bytes.index_from channel.buffer channel.b_start char 
    with Not_found -> 
...
let rec read_size size channel  =
  if channel.b_end - channel.b_start < size then Ok(channel) 
...
let read_fix_message channel =
  ...
  | None -> Result.error `BadSizeField 
  | Some size -> 
