...
let receive_engine t event = 
  ...
  | Engine.FIXFromEngine message , Some fixio ->
    ...
    ; t.recv ( mk_event message Outgoing )
    ]  
  | _ -> Lwt.return_unit 
...
let with_locks recv f =
  ...
  @@ fun () -> with_catch_disconnect recv f
...
let server_handler (t : t) (in_addr: Unix.sockaddr) (inch, outch) =
  ...
  if Lwt_mutex.is_locked connection_lock then t.recv (ConnectionRejected addr_str) else
...
let make_state_and_thread
  ...
  let engine_thread , engine = 
    ...
    in
  let state = 
    { fixio = None ; engine; engine_box; fixio_box; send_box; result_box; recv} in
  (state , engine_thread)   
