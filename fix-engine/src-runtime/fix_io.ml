...
module Internal : sig
  ...
  module Read : sig
    ...
    let next_split (self : t) : int option =
      ...
      | Some n when n >= self.max_off -> None
      | r -> r
    ...
    let refill (self : t) : bool Lwt.t =
      ...
      if self.max_off = B.length self.buf then (
    ...
    let rec read_available_pairs (self : t) : available_res =
      ...
      | Some idx_split ->
        ...
        | Some idx_eq when idx_eq < idx_split ->
          ...
          let v = B.sub_string self.buf (idx_eq + 1) (idx_split - idx_eq - 1) in
          ...
          if k = "10" then
            ...
            (read_available_pairs [@tailcall]) self
        | _ ->
          invalid_arg "Fix_io.read_next_message: no '=' present before split")
        ...
        | Needs_more ->
          ...
          if read_some then (
            ...
            if not (is_empty self) then
              invalid_arg
                (Printf.sprintf "Fix_io.read_next_message: leftover data %S"
