...
let create_logon_msg ( engine : fix_engine_state ) = 
    ...
    ValidMsg ( create_outbound_fix_msg ( engine, msg_data, false) )
;;
...
let create_session_reject_msg ( engine, reject_info : fix_engine_state * session_rejected_msg_data  ) = 
    let msg_data = 
        Full_FIX_Admin_Msg (
            Full_Msg_Reject {
...
let create_business_reject_msg ( engine, reject_info: fix_engine_state * biz_rejected_msg_data ) =
    let msg_data = 
        Full_FIX_Admin_Msg (
...
let proc_incoming_int_msg ( x, engine : fix_engine_int_msg * fix_engine_state) = 
    ...
    | ApplicationData ad    -> (
        ...
        | ActiveSession     -> 
            ...
            let msg = create_outbound_fix_msg (engine, app_msg_data, false) in { 
                engine with 
                    fe_last_time_data_sent = engine.fe_curr_time;
...
let session_reject ( rejected_data, engine : session_rejected_msg_data * fix_engine_state ) =
    let reject_msg = create_session_reject_msg ( engine, rejected_data ) in {
        engine with 
            incoming_fix_msg = None;
            fe_last_time_data_sent = engine.fe_curr_time;
            outgoing_fix_msg = Some reject_msg;
