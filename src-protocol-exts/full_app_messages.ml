...
type full_fix_heartbeat_data = {
    ...
    f_Heartbeat_TestReqID : fix_string option
}
;;
...
type full_fix_logon_data = {
    (** Method of encryption*)
    (** (Always unencrypted)*)
    f_Logon_EncryptMethod : fix_encryptmethod;
    (** Heartbeat interval (seconds)*)
    (** Note same value used by both sides*)
    f_Logon_HeartBtInt : int;
    (** Number of bytes in raw data field.*)
    (** Required for some authentication methods*)
    f_Logon_RawDataLength : int option;
    (** Unformatted raw data, can include bitmaps, word processor documents, etc.*)
    (** Required for some authentication methods*)
    f_Logon_RawData : fix_string option;
    ...
    f_Logon_ResetSeqNumFlag : fix_resetseqnumflag option;
    ...
    f_Logon_NextExpectedMsgSeqNum : int option;
    ...
    f_Logon_RefMsgType : fix_string option;
    ...
    f_Logon_MsgDirection : fix_msgdirection option;
    ...
    f_Logon_TestMessageIndicator : fix_testmessageindicator option;
    (** Userid or username.*)
    f_Logon_Username : fix_string option;
    (** Password or passphrase.*)
    (** Note: minimal security exists without transport-level encryption.*)
    f_Logon_Password : fix_string option
}
;;

(** The logout message initiates or confirms the termination of a FIX session.*)
type full_fix_logout_data = {
    (** Free format text string*)
    (** (Note: this field does not have a specified maximum length)*)
    f_Logout_Text : fix_string option;
    (** Byte length of encoded (non-ASCII characters) EncodedText (355) field.*)
    (** Must be set if EncodedText field is specified and must immediately precede it.*)
    f_Logout_EncodedTextLen : int option;
    (** Encoded (non-ASCII characters) representation of the Text (58) field in the encoded format specified via the MessageEncoding (347) field. If used, the ASCII (English) representation should also be specified in the Text field.*)
    (** Encoded (non-ASCII characters) representation of the Text field in the encoded format specified via the MessageEncoding field.*)
    f_Logout_EncodedText : fix_string option
}
;;
...
type full_fix_reject_data = {
    (** Reference message sequence number*)
    (** MsgSeqNum of rejected message*)
    f_Reject_RefSeqNum : int;
    (** The tag number of the FIX field being referenced.*)
    (** The tag number of the FIX field being referenced.*)
    f_Reject_RefTagID : int option;
    (** The MsgType (35) of the FIX message being referenced.*)
    (** The MsgType of the FIX message being referenced.*)
    f_Reject_RefMsgType : fix_string option;
    (** Code to identify reason for a session-level Reject message*)
    (** Code to identify reason for a session-level Reject message.*)
    f_Reject_SessionRejectReason : fix_sessionrejectreason option;
    (** Free format text string*)
    (** (Note: this field does not have a specified maximum length)*)
    (** Where possible, message to explain reason for rejection*)
    f_Reject_Text : fix_string option;
    (** Byte length of encoded (non-ASCII characters) EncodedText (355) field.*)
    (** Must be set if EncodedText field is specified and must immediately precede it.*)
    f_Reject_EncodedTextLen : int option;
    (** Encoded (non-ASCII characters) representation of the Text (58) field in the encoded format specified via the MessageEncoding (347) field. If used, the ASCII (English) representation should also be specified in the Text field.*)
    (** Encoded (non-ASCII characters) representation of the Text field in the encoded format specified via the MessageEncoding field.*)
    f_Reject_EncodedText : fix_string option
}
;;
...
type full_fix_resendrequest_data = {
    (** Message sequence number of first message in range to be resent*)
    f_ResendRequest_BeginSeqNo : int;
...
type full_app_msg_data =
    ...
    | FIX_Full_Msg_Heartbeat of full_fix_heartbeat_data
    ...
    | FIX_Full_Msg_Logon of full_fix_logon_data
    | FIX_Full_Msg_Logout of full_fix_logout_data
    ...
    | FIX_Full_Msg_Reject of full_fix_reject_data
    ...
    | FIX_Full_Msg_ResendRequest of full_fix_resendrequest_data
    ...
    | FIX_Full_Msg_SequenceReset of full_fix_sequencereset_data
...
let get_full_app_msg_tag ( msg : full_app_msg_data ) =
    match msg with
        ...
        | FIX_Full_Msg_Heartbeat _ -> Full_Msg_Heartbeat_Tag
        ...
        | FIX_Full_Msg_Logon _ -> Full_Msg_Logon_Tag
        | FIX_Full_Msg_Logout _ -> Full_Msg_Logout_Tag
        ...
        | FIX_Full_Msg_Reject _ -> Full_Msg_Reject_Tag
        ...
        | FIX_Full_Msg_ResendRequest _ -> Full_Msg_ResendRequest_Tag
        ...
        | FIX_Full_Msg_SequenceReset _ -> Full_Msg_SequenceReset_Tag
