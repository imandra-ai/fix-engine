...
let new_cache =
  Fix_engine_transitions.update_cache
    (List.map
       (fun x -> Fix_engine_state.CacheMessage x)
       Test_cache_utils.cache )
...
let print_cache cache_entry =
  ...
  | Fix_engine_state.CacheMessage m ->
      Yojson.Basic.pretty_to_string
        (Full_messages_json.full_valid_msg_to_json m)
  | Fix_engine_state.CacheGap (l, r) ->
      Yojson.Basic.pretty_to_string
...
Test_cache_utils.check_cache
  ( List.map (fun x -> Fix_engine_state.CacheMessage x) Test_cache_utils.cache1
  , Test_cache_utils.test_cache_index )
;;

pe "\nFails - missing 5-9";;

Test_cache_utils.check_cache
  ( List.map (fun x -> Fix_engine_state.CacheMessage x) Test_cache_utils.cache2
  , Test_cache_utils.test_cache_index )
;;
...
Test_cache_utils.check_cache
  ( List.map (fun x -> Fix_engine_state.CacheMessage x) Test_cache_utils.cache3
  , Test_cache_utils.test_cache_index )
