...
let assert_eq_msg (m1 : Fix_io.message) m2 =
  if m1 <> m2 then (
    Printf.printf "different messages:\n  %s\n vs: %s\n%!"
      (Fix_io.show_message m1) (Fix_io.show_message m2);
    assert false
  )
...
module Receive = struct
      ...
      (let data = String.concat "" test_data in
       ...
       let* l2 =
         ...
         read_messages
           ...
           ~read_next_message:Fix_io.Read.read_next_message data
       in
       if l1 <> l2 then (
         Format.printf "parsed (ref): %a@."
           (Format.pp_print_list Fix_io.pp_message)
           l1;
         Format.printf "parsed (fix_io): %a@."
           (Format.pp_print_list Fix_io.pp_message)
           l2;
         assert false
       );
  ...
  let () =
    ...
    let ic =
      Lwt_io.of_bytes ~mode:Lwt_io.input
      @@ Lwt_bytes.of_string
           (Printf.sprintf "a=b|%s=%s|10=123|" (String.make 2000 'k')
              (String.make 2000 'v'))
    in
    let read = Fix_io.Read.create ~buf_size:32 ~split:'|' ic in
    let* m = Fix_io.Read.read_next_message read in
